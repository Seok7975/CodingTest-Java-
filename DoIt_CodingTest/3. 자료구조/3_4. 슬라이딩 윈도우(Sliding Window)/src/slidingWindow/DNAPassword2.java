...
public class DNAPassword2 {
    public static void main(String[] args) throws Exception {
        ...
        char[] DNAArr = bf.readLine().toCharArray(); // DNA 문자열을 char 배열로 변환

        st = new StringTokenizer(bf.readLine()); // 새로운 입력을 받기 위한 st 객체 생성
        for (int i = 0; i < 4; i++) {
            checkArr[i] = Integer.parseInt(st.nextToken()); // A C G T의 최소 갯수 설정해 배열에 저장
        }

        int correctCount = 0;
        // checkArr의 값이 0인 경우 correctCount 증가
        for (int i = 0; i < 4; i++) {
            if (checkArr[i] == 0) {
                correctCount++;
            }
        ...
        for (int i = 0; i < partsStringLength; i++) {
            correctCount = addDNACode(DNAArr[i], myDNAArr, checkArr, correctCount);
        }
        ...
        for (int i = partsStringLength; i < stringLength; i++) {
            ...
            correctCount = removeDNACode(DNAArr[j], myDNAArr, checkArr, correctCount);
            correctCount = addDNACode(DNAArr[i], myDNAArr, checkArr, correctCount);
    ...
    public static int addDNACode(char DNACode, int[] myDNAArr, int[] checkArr, int correctCount) {
        switch (DNACode) {
            case 'A':
                myDNAArr[0]++;
    ...
    public static int removeDNACode(char DNACode, int[] myDNAArr, int[] checkArr, int correctCount) {
        switch (DNACode) {
            case 'A':
                if (myDNAArr[0] == checkArr[0]) {
                    correctCount--;
